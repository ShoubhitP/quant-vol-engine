...
def heston_cos_price(S, K, T, r, kappa, theta, xi, rho, v0, option_type="call", N=100):
    F = S * np.exp(r * T)
...
def calibrate_heston(strikes, expiries, ivs, S, r):
    ...
    def loss(params):
        ...
        for strike, expiry in zip(strikes, expiries):
            heston_prices.append(heston_cos_price(S, strike, expiry, r, kappa, theta, xi, rho, v0, "call"))
        return np.sum(np.square(np.array(heston_prices) - np.array(market_prices)))

    result = optimize.minimize(loss, [2, 0.04, 0.3, -0.7, 0.04], bounds=[(0.001, 20), (0.001, 1), (0.001, 5), (-0.999, 0.999), (0.001, 1)])
    return result.x

def main():
    testCall1Val = heston_cos_price(100, 100, 1, 0.05, 2, 0.04, 0.3, -0.7, 0.04, "call", 100)
    print(f"{testCall1Val:.2f} is the call price for test 1.")
    testPutVal = heston_cos_price(100, 100, 1, 0.05, 2, 0.04, 0.3, -0.7, 0.04, "put", 100)
